
/*
 Utilities to explore AST
 - http://dferber90.github.io/graphql-ast-explorer/
 - http://astexplorer.net/
 */

const { inspect } = require('util');

// Note: Fragments are not supported, so ast.fieldNodes.length === 1.
module.exports = function graphqlResolverAst(ast) {
  inspector(ast, 3)

  const state = {
    operation: ast.schema._queryType ? 'Query' : 'Mutation',
    schema: [ast.parentType.name, ast.path.key],
    resolverPath: [],
    resolveTo: ast.returnType,
    fields: [{ resolverPath: [], selections: [] }], // So we can enhance to support fragments.
  };

  let path = ast.path;
  const resolverPath = [ast.parentType.name, path.key];

  while (path.prev) {
    path = path.prev;
    resolverPath.unshift(path.key);
  }

  state.resolverPath = resolverPath;

  // selection
  const selects = ast.fieldNodes[0].selectionSet.selections;
  state.fields[0].selections = selects.map(
    sel => ({ name: sel.name.value, hasSelections: !!sel.selectionSet })
  );

  // args
  const args1 = {};

  ast.fieldNodes[0].arguments.forEach(argument => {
    args1[argument.name.value] = getArgumentValue(argument.value);
  });

  state.args = Object.keys(args1) ? args1 : null;
  return state;
}

function getArgumentValue(argumentValue) {
  switch (argumentValue.kind) {
    case 'StringValue': // fall through
    case 'BooleanValue':
      return argumentValue.value;
    case 'IntValue':
      return parseInt(argumentValue.value, 10);
    case 'FloatValue':
      return parseFloat(argumentValue.value);
    case 'ListValue':
      return argumentValue.values.map(elem => getArgumentValue(elem));
    case 'ObjectValue':
      const obj = {};
      argumentValue.fields.forEach(field => {
        obj[field.name.value] = getArgumentValue(field.value);
      });
      return obj;
    default:
      throw new Error(`Unexpected GraphQL argument type "${argumentValue.kind}"`);
  }
}

function inspector(obj, depth) {
  console.log(inspect(obj, { depth, colors: true }));
}
